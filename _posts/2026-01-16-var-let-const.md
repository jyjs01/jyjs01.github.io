...
JavaScript에서 변수를 선언하는 방법은 `var`, `let`, `const`가 있다.  
요즘은 **대부분 `let`과 `const`를 사용**하고, `var`는 레거시 코드에서 주로 만난다.
...
`var`는 `if`, `for` 같은 **블록 `{}`을 무시**하고, **함수 단위**로만 영역이 나뉜다.
...
다만 `const`로 선언한 **객체/배열**은 내부 값 변경이 가능하다.  
...
그래서 `var`는 의도치 않은 버그를 만들기 쉽다. <br /><br />
